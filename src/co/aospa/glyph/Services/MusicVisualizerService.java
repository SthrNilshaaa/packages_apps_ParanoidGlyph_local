...
public class MusicVisualizerService extends Service {
    ...
    private double mRunningSoundAvg[]; // Total sound energy in one second (0=low, 1=mid low, 2=mid, 3=mid high,
                                       // 4=high)
    private double mCurrentAvgEnergyOneSec[]; // Average sound energy in one second (0=low, 1=mid low, 2=mid, 3=mid
                                              // high, 4=high)
    ...
    public void onCreate() {
        ...
        // Create a visualizer with the audio session ID (0) which takes the entire
        // output mix
    ...
    private void processAudioFFT(byte[] audioBytes, int samplingRate) {
        ...
        // Also make sure the mCurrentAvgEnergy has been set, otherwise its -1 before
        // its first pass
        if ((sampleAvgAudioEnergy > mCurrentAvgEnergyOneSec[0]) && (mCurrentAvgEnergyOneSec[0] > 0)) {
            if (DEBUG)
                Log.d(TAG, "Low frequency band beat detected");
            AnimationManager.playMusic(MusicVisualizerService.this, "low");
        }
        ...
        if ((sampleAvgAudioEnergy > mCurrentAvgEnergyOneSec[1]) && (mCurrentAvgEnergyOneSec[1] > 0)) {
            if (DEBUG)
                Log.d(TAG, "Mid-low frequency band beat detected");
            AnimationManager.playMusic(MusicVisualizerService.this, "mid_low");
        }
        ...
        if ((sampleAvgAudioEnergy > mCurrentAvgEnergyOneSec[2]) && (mCurrentAvgEnergyOneSec[2] > 0)) {
            if (DEBUG)
                Log.d(TAG, "Mid frequency band beat detected");
            AnimationManager.playMusic(MusicVisualizerService.this, "mid");
        }
        ...
        if ((sampleAvgAudioEnergy > mCurrentAvgEnergyOneSec[3]) && (mCurrentAvgEnergyOneSec[3] > 0)) {
            if (DEBUG)
                Log.d(TAG, "Mid-high frequency band beat detected");
            AnimationManager.playMusic(MusicVisualizerService.this, "mid_high");
        }
        ...
        if ((sampleAvgAudioEnergy > mCurrentAvgEnergyOneSec[4]) && (mCurrentAvgEnergyOneSec[4] > 0)) {
            if (DEBUG)
                Log.d(TAG, "High frequency band beat detected");
            AnimationManager.playMusic(MusicVisualizerService.this, "high");
        }
